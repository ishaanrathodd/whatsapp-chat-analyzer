...
def fetch_stats(selected_user, media_df):
    ...
    words = []
    for message in media_df['message']:
        if not any(substring in message for substring in
                   ['.heic', '.png', 'image omitted', 'video omitted', '.mp4', '.jpg', '.jpeg', '.mov',
    ...
    num_images = media_df[media_df['message'].str.contains('image omitted|.heic|.png|.jpeg|.jpg', na=False)].shape[0]
    num_videos = media_df[media_df['message'].str.contains('video omitted|mov|mp4', na=False)].shape[0]
    num_audios = media_df[media_df['message'].str.contains('audio omitted')].shape[0]
    ...
    return num_messages, len(words), num_videos, num_images, num_stickers, num_audios, len(links)
